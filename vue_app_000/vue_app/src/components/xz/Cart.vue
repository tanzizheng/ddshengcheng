<template>
	...
	<div class="cart">
		...
		<div v-for="(item,index) of list" :key="index" class="cart-item">
			<div class="leftText">
				<input type="checkbox" v-model="item.cd" @click="selectitem" />
		...
		<div>
			...
			<mt-button size="large" @click="deleteItems">删除选中的<span style="color:red">{{Cart_const}}</span>个商品</mt-button>
	...
	export default {
		...
		methods: {
			...
			selectAll(event) {
				...
				if(cb){
					this.Cart_const=this.list.length;
				}else{
					this.Cart_const=0;
				}
